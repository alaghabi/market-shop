        ...
        <AnimatePresence>
          {open && (
            <>
              ...
              />
              <motion.div
                className="fixed inset-y-0 right-0 z-50 flex h-[100dvh] min-h-0 w-[min(100vw,36rem)] max-w-full flex-col overflow-hidden border-l border-[color:var(--sf-outline,var(--ds-outline-variant))] bg-[color:var(--sf-surface,var(--ds-surface))] text-[color:var(--sf-text,var(--ds-on-surface))] shadow-2xl"
                role="dialog"
                aria-modal="true"
              ...
              >
                <div className="flex items-center justify-between border-b border-[color:var(--sf-outline,var(--ds-outline-variant))] px-4 py-4 sm:px-6 sm:py-5">
                ...
                <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-4 py-4 sm:px-6 sm:py-5">
                  ...
                  {items.map((i) => (
                    <div key={cartItemKey(i)} className="grid min-w-0 grid-cols-[5rem_minmax(0,1fr)] gap-x-3 gap-y-3 border-b border-[color:var(--sf-outline,var(--ds-outline-variant))] pb-5">
                      <div className="h-20 w-20 shrink-0 overflow-hidden rounded-xl bg-[color:var(--sf-surface-muted,var(--ds-surface-container))] sm:h-24 sm:w-24">
                        {(() => {
                      ...
                      <div className="col-span-2 flex min-w-0 items-center justify-between gap-3">
                        ...
                        <button type="button" onClick={() => onRemove(cartItemKey(i))} className="sf-neutral-action shrink-0 cursor-pointer rounded-full px-3 py-2 text-xs transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500">Retirer</button>
                        <div className="shrink-0 text-right text-sm font-bold text-[color:var(--sf-text,var(--ds-on-surface))]">{(i.qty * i.product.priceCents / 100).toFixed(2)} {i.product.currency}</div>
                      </div>
                    </div>
                  ))}
                </div>

                <div className="space-y-3 border-t border-[color:var(--sf-outline,var(--ds-outline-variant))] px-4 py-4 sm:px-6 sm:py-5">
                  ...
                  <a href={boutiqueLink('/cart')} className={`sf-primary-action flex w-full items-center justify-center rounded-xl px-4 py-3 text-sm font-bold transition-colors ${items.length === 0 ? 'pointer-events-none bg-gray-300' : 'bg-[color:var(--sf-accent,var(--ds-primary,#111111))] hover:opacity-90'}`}>
                    Passer commande
                  </a>
                </div>
              </motion.div>
